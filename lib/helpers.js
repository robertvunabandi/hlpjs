...
const hlp = {
  ...
  /**
   * Simply throw an error with constructor ErrorInstance with the
   * given message. This will only throw if the ErrorInstance prototype
   * is an instance of Error (i.e. is not inherited from Error).
   *
  ...
  throw (message, ErrorInstance = Error) {
    // check the ErrorInstance in case the ErrorInstance is null
    if (ErrorInstance && ErrorInstance.prototype instanceof Error) {
  ...
  */
  ...
  /**
   * @tested
   * Simply check if the given argument is a boolean.
   * A note on this is that it works only for primitive booleans
   *
   * @param {Object} arg
   * @return {Boolean}
   **/
   ...
   * i.e. String, Number, null, Boolean, or undefined.
   *
   * @param {Object} arg
   * @return {Boolean}
   **/
  isPrimitive (arg) {
    return hlp.isBoolean(arg) ||
      hlp.isString(arg) ||
      hlp.isNumber(arg) ||
      hlp.isNull(arg) ||
      hlp.isUndefined(arg);
   ...
   **/
  Primitive (arg) {
    let err_msg = 'arg must be of Primitive type to be declared primitive';
    hlp.assert.argIsPrimitive(arg, err_msg);
    this.el = arg;
  },
  ...
  */
  ...
  hasStringLength (str, exp_len) {
    ...
    let msg = 'Invalid parameter args: ';
    msg += 'must be String for string and Number for exp_len';
    hlp.throw(msg, TypeError);
  },
  /**
   * @tested
  ...
  extendString (desired_length, str_or_num, addition_character = ' ') {
    ...
    let err_msg = 'Invalid type argument for desired_length: must be int';
    hlp.assert.argIsInteger(desired_length, err_msg, TypeError);
    ...
    let str_err_msg = 'addition_character must be a String';
    hlp.assert.argIsString(addition_char, str_err_msg, TypeError);
    let add_err_msg = 'addition_character must be a string of length 1';
    hlp.assert.stringHasLength(addition_char, 1, add_err_msg);
   ...
   * @param {String} str
   * @return {Number}
   * @throws {Error} if the argument is not a String or if str
  ...
  /**
   * @tested
   * checks if the Primitive ags is contained in the array of Primitive values
   *
   *
   * @param {hlp.Primitive} primitive
   * @param {Array<hlp.Primitive>} arr
   * @return {Boolean}
   * @throws {Error} if primitive1 is not Primitive or the array contains
   *   a non-primitive values
   **/
  isPrimitiveArgInArray (primitive, arr) {
    function candidateErrMsg(candidate) {
      return `candidate (${candidate}) must be of Primitive type`;
    }
    hlp.assert.argIsPrimitive(primitive, 'arg must be of Primitive type', TypeError);
    for (let candidate of arr) {
      hlp.assert.argIsPrimitive(candidate, candidateErrMsg(candidate), TypeError);
  ...
  /**
   * @tested
   * checks if the Primitive arg is contained in the array of Primitive values
   *
  ...
  assert: {
    ...
    argIsNumber (arg, message = null, ErrorInstance = Error) {
      if (!hlp.isNumber(arg)) {
        hlp.throw(message || 'arg was not a number', ErrorInstance);
      }
    },
    /**
     * @param {Object} arg
    ...
    argIsInteger (arg, message = null, ErrorInstance = Error) {
      if (!hlp.isInteger(arg)) {
        hlp.throw(message || 'arg was not an integer', ErrorInstance);
      }
    },
    /**
     * @param {Object} arg
    ...
    argIsNull (arg, message = null, ErrorInstance = Error) {
      if (!hlp.isNull(arg)) {
        hlp.throw(message || 'arg was not null', ErrorInstance);
      }
    },
    /**
     * @param {Object} arg
    ...
    argIsNotNull (arg, message = null, ErrorInstance = Error) {
      if (hlp.isNull(arg)) {
    ...
    argIsNotUndefined (arg, message = null, ErrorInstance = Error) {
      if (hlp.isUndefined(arg)) {
        hlp.throw(message || 'arg was undefined', ErrorInstance);
      }
    },
    /**
     * @param {Object} arg
    ...
    argIsString (arg, message = null, ErrorInstance = Error) {
      if (!hlp.isString(arg)) {
        hlp.throw(message || 'arg was not a String', ErrorInstance);
      }
    },
    /**
     * @param {Object} arg
    ...
    argIsArray (arg, message = null, ErrorInstance = Error) {
      if (!hlp.isArray(arg)) {
        hlp.throw(message || 'arg was not an Array', ErrorInstance);
      }
    },
    /**
     * @param {Object} arg
    ...
    argIsPrimitive (arg, message = null, ErrorInstance = Error) {
      if (!hlp.isPrimitive(arg)) {
        hlp.throw(message || 'arg was Primitive', ErrorInstance);
      }
    },
    /**
     * @param {String} str
    ...
    stringHasLength (str, exp_len, message = null, ErrorInstance = Error) {
      if (!hlp.hasStringLength(str, exp_len)) {
        let default_msg = `string ${str} does not have length ${exp_len}`;
        hlp.throw(message || default_msg, ErrorInstance);
      }
    },
    /**
     * @param {String} str
    ...
    stringContainsOnlyNumeric (str, message = null, ErrorInstance = Error) {
      if (hlp.containsNonNumeric(str)) {
        let default_msg = `string ${str} contains non-numeric characters`;
        hlp.throw(message || default_msg, ErrorInstance);
      }
    },
    /**
    ...
    primitivesAreEqual (primitive1, primitive2, message = null, ErrorInstance = Error) {
      if (!hlp.arePrimitiveArgsEqual(primitive1, primitive2)) {
        let default_msg = `${primitive1} and ${primitive2} are not equal`;
        hlp.throw(message || default_msg, ErrorInstance);
      }
    }
  },
